 ...
 * Adjusted to use RenderedFrame by Unnamed Studios
 */
...
namespace UnnamedStudios
{
	public class GifEncoder
	{
		...
		protected RenderedFrame m_CurrentFrame;
		...
		public void AddFrame(RenderedFrame frame)
		{
		...
		public void BuildPalette(RenderedFrame[] frames, int frameCount)
		{
			...
			byte[] combinedPixels = new byte[3 * frame.Width * frame.Height * (1 + frameCount / m_FramesPerColorSample)];
			int count = 0;

			// Stich the large image together out of pixels from several frames
			for (int i = 0; i < frameCount; i += m_FramesPerColorSample)
			{
				frame = frames[i];
				Color32[] p = frame.Colors;
			...
			nq = new NeuQuant(combinedPixels, combinedPixels.Length, m_SampleInterval);
		...
		protected void GetImagePixels()
		{
			...
			Color32[] p = m_CurrentFrame.Colors;
			int count = 0;
		...
		protected void AnalyzePixels()
		{
			...
			if (m_FramesPerColorSample == 0)
			{
				nq = new NeuQuant(m_Pixels, len, m_SampleInterval);
